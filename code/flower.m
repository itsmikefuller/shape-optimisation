...
f_ = @(x,y) sqrt(x.^2+y.^2)-[cos(7*tan(y./x))+2.5]; %Integrand
...
plot(real(g(t2)), imag(g(t2)), 'b'); %Initial boundary
...
%fn = chebfun(@(t) n(t).*f(real(g(t)), imag(g(t))), [0 2*pi,1e3]); %Search direction
  ...
  for k=1:50
    ...
    figure(2)
        ...
        %subplot(2,2,2);
        plot(abs(diff(g2))/sum(abs(diff(g2)))*2*pi);title('Slope dR/dt');
        %subplot(2,2,3);plot(real(fn(R(t2))));title('Re[fn(R(t))]');
        %subplot(2,2,4);plot(imag(fn(R(t2))));title('Im[fn(R(t))]');
